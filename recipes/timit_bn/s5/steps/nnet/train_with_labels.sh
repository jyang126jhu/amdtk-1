...
num_tgt=300           # (optional) specifiy number of NN outputs, to be used with 'labels=',
...
data=$1 # it's actually should be data_tr
data_cv=$2
dir=$3
...
if [ ! -z "$labels_tr" ] && [ ! -z $labels_cv ]; then
  ...
  echo "Need labels (posts for the training)" && exit 1;
fi
...
#### Force to use cmvn per utt #####
  echo "# + 'apply-cmvn' with '$cmvn_opts' using statistics : $data/cmvn.scp, $data_cv/cmvn.scp"
  [ ! -r $data/cmvn.scp ] && echo "Missing $data/cmvn.scp" && exit 1;
  [ ! -r $data_cv/cmvn.scp ] && echo "Missing $data_cv/cmvn.scp" && exit 1;
  feats_tr="$feats_tr apply-cmvn  scp:$data/cmvn.scp ark:- ark:- |"
  feats_cv="$feats_cv apply-cmvn  scp:$data_cv/cmvn.scp ark:- ark:- |"
############ 2016-07-26 #############
...
if [ ! -z $nnet_init ]; then
  ...
  # output-dim,
  [ -z $num_tgt ] && \
    echo "Need to specify the num_tgt" && exit 1;
